...
RAM 用 AT28C16 实现的方法: 添加一个比较器, 当地址不在规定的内存范围内时, 将 $\overline{CE}$ 置1, 芯片失能, I/O = Z. 总线上的 WE 直接接 $\overline{OE}$, 当地址在范围内时, 如果 WE = 0 (可能需要将 AT28C16 的 $\overline{WE}$ 强制置1), 正常读取, I/O 输出到数据总线上. 如果 WE = 1, 写入操作需要 $\overline{WE}$ 上的一个负脉冲驱动. 可以考虑将系统时钟 CLK 接 $\overline{WE}$​ ,并且将 CLK 调整为具有极高占空比 (根据AT28C16要求, 低电平时间0.1~1us), 用 CLK 驱动写入.
